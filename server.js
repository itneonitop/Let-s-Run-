...
io.on('connection', (socket) => {
  ...
  const removePlayerFromRoom = (socketId, targetRoomCode) => {
    for (const code in rooms) {
      if (targetRoomCode && code !== targetRoomCode) continue;
      ...
      if (room && room.players[socketId]) {
        ...
        if (remainingPlayers.length === 0) {
          ...
          io.to(code).emit('player-left', { id: socketId, name: leavingPlayer.name });
          io.to(code).emit('room-updated', room);

          if (room.started && !room.ended) {
            const stillIn = remainingPlayers.filter(p => !p.lost && !p.won);
            if (stillIn.length === 1 && remainingPlayers.length > 1) {
              const winnerId = stillIn[0].id;
              room.players[winnerId].won = true;
              room.ended = true;
              io.to(code).emit('game-won-broadcast', { id: winnerId, name: room.players[winnerId].name });
              io.to(code).emit('room-updated', room);
  ...
  socket.on('create-room', ({ name, maxPlayers, mode, teamsCount, isPublic }) => {
    const cleanName = (name || 'Racer').trim();
    const roomCode = Math.random().toString(36).substring(2, 8).toUpperCase();
    const seed = Math.floor(Math.random() * 1000000);
    rooms[roomCode] = {
      players: {
        [socket.id]: { id: socket.id, name: cleanName, ready: false, color: '#ff4400', pos: [0, 1, 0], team: 0, progress: 0 }
      },
  ...
  socket.on('join-room', ({ name, roomCode }) => {
    ...
    if (!cleanName) {
      socket.emit('error', 'Введите никнейм!');
      return;
    }
    ...
    if (room) {
      ...
      if (nameTaken) {
        socket.emit('error', `Ник «${cleanName}» уже занят в этой комнате! Выберите другой ник.`);
        return;
      }

      if (Object.keys(room.players).length < room.settings.maxPlayers) {
        room.players[socket.id] = { id: socket.id, name: cleanName, ready: false, color: '#0077ff', pos: [0, 1, 0], team: 1, progress: 0 };
        socket.join(cleanCode);
        socket.emit('sync-items', room.pickedItems);
        socket.emit('room-created', { roomCode: cleanCode, room });
        io.to(cleanCode).emit('player-joined', { id: socket.id, name: cleanName });
  ...
  socket.on('quick-play', ({ name }) => {
    ...
    const availableRooms = Object.entries(rooms).filter(([code, room]) => {
      const notFull = Object.keys(room.players).length < room.settings.maxPlayers;
      const nameTaken = Object.values(room.players).some(
        (p) => (p.name || '').trim().toLowerCase() === cleanName.toLowerCase()
      );
      return room.settings.isPublic && !room.started && notFull && !nameTaken;
    });

    if (availableRooms.length > 0) {
      const [roomCode] = availableRooms[Math.floor(Math.random() * availableRooms.length)];
      const room = rooms[roomCode];
      room.players[socket.id] = { id: socket.id, name: cleanName, ready: false, color: '#00ff44', pos: [0, 1, 0], team: 1 };
      socket.join(roomCode);
      socket.emit('sync-items', room.pickedItems);
      socket.emit('room-created', { roomCode, room });
      io.to(roomCode).emit('player-joined', { id: socket.id, name: cleanName });
      io.to(roomCode).emit('room-updated', room);
      broadcastPublicRooms();
    } else {
      socket.emit('error', 'Нет свободных открытых комнат с таким ником. Создайте свою!');
    }
  });
